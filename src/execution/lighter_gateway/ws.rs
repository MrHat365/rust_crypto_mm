...
pub(super) enum LighterWsCommand {
    SendBatch {
...
#[derive(Clone)]
pub(super) struct LighterWsConfig {
    pub(super) ws_url: String,
    pub(super) account_index: i64,
    pub(super) api_key_index: i32,
    pub(super) market_index: u8,
    pub(super) debug_prints: bool,
    pub(super) suppress_sendtx_quota_logs: bool,
}

pub(super) struct LighterWsWorker {
    cfg: LighterWsConfig,
    signer: SignerHandle,
...
impl LighterWsWorker {
    fn should_suppress_sendtx_quota_log(&self, err_msg: &str) -> bool {
    ...
    pub(super) fn extract_sendtx_req_id(value: &Value) -> Option<String> {
        value
            .get("data")
            .and_then(|d| d.get("id"))
    ...
    pub(super) fn parse_sendtx_response(value: &Value) -> Result<SendTxBatchResponse> {
    ...
    pub(super) fn parse_sendtx_error(value: &Value) -> Result<anyhow::Error> {
    ...
    pub(super) fn value_has_sendtx_marker(value: &Value) -> bool {
        let msg_type = value
            .get("type")
            .and_then(|v| v.as_str())
            .unwrap_or_default();
    ...
    pub(super) fn insert_pending_batch(
        pending: &mut HashMap<String, PendingSendTxBatch>,
        pending_by_hash: &mut HashMap<String, String>,
        req_id: String,
    ...
    pub(super) fn complete_pending_batch(
        pending: &mut HashMap<String, PendingSendTxBatch>,
        pending_by_hash: &mut HashMap<String, String>,
        req_id: &str,
    ...
    pub(super) fn parse_account_tx_event_info(tx: &LighterTxEntry) -> Result<Option<Value>> {
    ...
    pub(super) fn collect_account_tx_order_refs(
        tx_type: u8,
    ...
    pub(super) fn update_pending_batch_from_account_tx(
        tx: &LighterTxEntry,
        pending: &mut HashMap<String, PendingSendTxBatch>,
        pending_by_hash: &HashMap<String, String>,
    ...
    fn handle_pending_batch_rejection(
        ...
        if !self.should_suppress_sendtx_quota_log(&err_text) {
            eprintln!(
                "WARN: lighter sendTxBatch async rejection req_id={} err={} tx_meta={:?}",
                req_id, err_text, batch.tx_meta
            );
        }
    ...
    pub(super) fn validate_sendtx_response_hashes(
        req_id: &str,
    ...
    pub(super) async fn await_initialization_message<S>(
