...
use crate::exchanges::weex::orderbook::{PRICE_SCALE, QTY_SCALE};
...
impl<const N: usize> WeexEngine<N> {
    pub fn new(symbol: String, consumer: Consumer<WeexFrame, N>) -> Self {
        Self {
            consumer,
            pending: None,
    ...
    pub fn process(
        ...
        if let Some(mut f) = self.pending.take().or_else(|| self.consumer.try_pop().ok()) {
            ...
            let ts = f.ts;
            {
                for (feed, _) in weex::events_for(text, &mut self.book) {
                    if feed == "orderbook" {
                ...
                if weex::update_bbo_store(text, &mut self.bbo) {
                ...
                let new_trades = weex::update_trades(text, &mut self.trades);
                if new_trades > 0 {
                ...
                if let Some((_, ticker)) = weex::update_tickers(text, &mut self.tickers) {
                    let mut st = lock_state();
