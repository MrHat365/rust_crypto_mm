...
use crate::exchanges::digifinex::DigifinexFrame;
use crate::exchanges::digifinex::orderbook::{PRICE_SCALE, QTY_SCALE};
...
impl<const N: usize> DigifinexEngine<N> {
    pub fn new(symbol: String, consumer: Consumer<DigifinexFrame, N>) -> Self {
        Self {
            consumer,
            pending: None,
    ...
    fn is_bbo_frame(frame: &DigifinexFrame) -> bool {
        frame.event() == Some("ticker.update")
    }
    ...
    pub fn process(
        ...
        if let Some(mut f) = self.pending.take().or_else(|| self.consumer.try_pop().ok()) {
            ...
            let ts = f.ts;
            {
                for (feed, _) in digifinex::events_for(text, &mut self.book) {
                    if feed == "orderbook" {
                ...
                if digifinex::update_bbo_store(text, &mut self.bbo) {
                    ...
                    if let Some(bbo) = entry {
                        ...
                        match feed_gate.evaluate(ExchangeFeed::Digifinex, FeedKind::Bbo, bbo_ts) {
                            GateDecision::Accept => {
                                demean.record_other(ExchangeKind::Digifinex, Some(bbo_ts), Some(mid));
                ...
                let new_trades = digifinex::update_trades(text, &mut self.trades);
                if new_trades > 0 {
                    for trade in self.trades.iter_last(new_trades) {
                        ...
                        match feed_gate.evaluate(
                            ...
                            GateDecision::Accept => {
                                let px = (trade.px as f64) / PRICE_SCALE;
                                let qty = (trade.qty as f64).abs() / QTY_SCALE;
                ...
                if let Some((_, ticker)) = digifinex::update_tickers(text, &mut self.tickers) {
                    let mut st = lock_state();
