        ...
        if let Some(mut f) = self.pending.take().or_else(|| self.consumer.try_pop().ok()) {
                ...
                && digifinex::update_bbo_store(&mut f, &mut self.bbo, self.qty_multiplier)
            {
                if let Some(mid) = self
                    .bbo
                    .mid_price_f64_for(&self.instrument_id)
            ...
            if event == "ticker.update" {
                    ...
                    digifinex::update_tickers(&mut f, &mut self.tickers, self.qty_multiplier)
                {
                    ...
                    if ticker.ticker.last_px != 0 {
                        entry.last_price = Some((ticker.ticker.last_px as f64) / digifinex::PRICE_SCALE);
                    }
                    if ticker.ticker.last_qty != 0 {
                        entry.last_qty = Some((ticker.ticker.last_qty as f64) / digifinex::QTY_SCALE);
                    }
                    if ticker.ticker.best_bid != 0 {
                        entry.best_bid = Some((ticker.ticker.best_bid as f64) / digifinex::PRICE_SCALE);
                    }
                    if ticker.ticker.best_ask != 0 {
                        entry.best_ask = Some((ticker.ticker.best_ask as f64) / digifinex::PRICE_SCALE);
                    }
                    ...
                    if let Some(last_ts) = entry.ts_ns {
                        if ticker_ts < last_ts {
                            eprintln!(
                                "WARN: dropping stale digifinex ticker update: ts={} last_ts={}",
                                ticker_ts, last_ts
                            );
                        } else {
